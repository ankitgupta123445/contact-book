...
Create Clients for App Databases
...
def get_mongo_db(config):
    m_db = []
    master_config = config['MONGO']
    for mcfg in master_config:
        m = []
        if mcfg.get('is_replica'):

            # 0 is Default PRIMARY
            m.append(MongoClient('mongodb://{}'.format(",".join(mcfg['replica_sets'])),
                                 replicaset=mcfg['replica_set_name'])[mcfg['db']])

            # 1 is SECONDARY_PREFERRED
            m.append(MongoClient('mongodb://{}'.format(",".join(mcfg['replica_sets'])),
                                 replicaset=mcfg['replica_set_name'],
                                 read_preference=ReadPreference.SECONDARY_PREFERRED)[mcfg['db']])

        else:
            for i in range(2):
                m.append(MongoClient(mcfg['host'], mcfg['port'], maxPoolSize=20000)[mcfg['db']])
...
EXECUTABLE_MONGO_METHODS = get_methods(pymongo.collection.Collection, pymongo)


class Executable:
    """ Wrap a MongoDB-method and handle AutoReconnect-exceptions
    using the safe_mongo call decorator.
    """
